...
#best_fitness_list
#B1 hm hr dafe update mishavad ps byd joze vorodi bashad
def initial_sol(Ii,B,n,Ui,ri,H):
    B1=B
    Ii0=Ii.copy()
    inv=np.zeros((H,n+1))
...
def swap_mutation1(s_inv,s_trans,H):
    random_horizen=np.random.randint(0,H,int(np.round(H*0.5)))
    #print('random_horizen=',random_horizen)
    s=s_inv.copy()
    trans=s_trans.copy()
    t=trans[random_horizen[0]].copy()
    t1=trans[random_horizen[1]].copy()
    a,b=np.random.randint(1,len(t),2)
    m=0
    #print("a=",a,"b=",b)
    if a!=b:
    ...
    a1,b1=np.random.randint(1,len(t1),2)    
    n=0
    #print("a=",a,"b=",b)
    if a1!=b1:
        n=t1[a1]
        ...
        t1[b1]=n    
    #print(t)    
    trans[random_horizen[0]]=t
    trans[random_horizen[1]]=t1
    #print(s_trans)
...
def recombination(par1,par2,Ii0,U):
    ...
    for i in range(len(choromosome1)):
        ...
        if child1[i] not in choromosome1:
            chorom1[0][random_horizen1][child1[i]]=Ui[child1[i]-1]-Ii[child1[i]-1]
    ...
    for i in range(len(choromosome2)):
        ...
        if child2[i] not in choromosome2:
            chorom2[0][random_horizen2][child2[i]]=Ui[child2[i]-1]-Ii[child2[i]-1]  
...
def GA(n,H,C,x1,y1,B1,r1,h1,X,Y,Ii,Ui,Li,ri,hi,third):
    population_inv=[]
    population_trans=[]
    for i in range(100):
        s,t=initial_sol(Ii,B1,n,Ui,ri,H)
    ...
    for i in range(len(best_fitness_list)):
        if check_valid(best_solutions_list[i][0],best_solutions_list[i][1],Ii,B1,r1,ri,Ui)==True:
            valid_list.append(best_fitness_list[i])
            valid_andis.append(i)
    best=min(valid_list)
...
df_marks = pd.DataFrame({'Instance':Instance_arr[ :63],'best': best_array,'worst':worst_array,'avg':mean_array,'avg_time':avgtime_array})
writer = pd.ExcelWriter('GA_OU_IRP_run20.xlsx')
