...
def s0_ML(Ui,Ii0):
    ...
    for i in range(len(Ui)):
        while(flag):
            x=np.random.randint(Ui[i]-Ii0[i],Ui[i])
        ...
        if q<=q0:
            for i in range(count_visited):
                if len(s_trans)==count_visited+1:
                    ...
                    to_eta=[]
                    for j in d:
                        to_eta.append(((pherom[s_trans[i],j])**alpha)*(1/(find_cij(s_trans[i],j,third)+0.1)**beta))
                    #print('to_eta=',to_eta)    
                    ...
                    pherom[y,s_trans[i]]=(1-Rho)*pherom[y,s_trans[i]]+(Rho*(1/(find_cij(y,s_trans[i],third)+0.1)))
                    #print('pherom=',pherom)
                    d.remove(y)
                    #print('d=',d)
        else:
...
for i in range(len(adresses)):
    ...
    val=fitness(inv_pop[0],trans_pop[0],B1,r1,h1,hi,Ii,ri,third)
    #pherom=np.zeros((n+1,n+1))
    #pherom.fill(1/val)
    #np.fill_diagonal(pherom, 0, wrap=False)
    if n<=30:
        v1=20
...
writer = pd.ExcelWriter('ACS_ML_IRP_lowcost.xlsx')
